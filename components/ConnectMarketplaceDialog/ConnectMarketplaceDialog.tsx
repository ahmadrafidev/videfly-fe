...
export function ConnectMarketplaceDialog({ onConnect }: { onConnect: () => void }) {
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="default">Tautkan Marketplace</Button>
      </DialogTrigger>
      <DialogContent className="max-w-sm p-0 overflow-hidden">
        <div className="flex items-center justify-between p-4">
          ...
          <DialogClose asChild>
            <button aria-label="Close" className="text-foreground/50 hover:text-foreground">×</button>
          </DialogClose>
        </div>
          ...
          {MARKETPLACES.map((m) => (
            <div key={m.name} className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  alert(`Berhasil menghubungkan ${m.name}`)
                  onConnect()
                }}
              >
                Tautkan
